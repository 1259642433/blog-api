...
func Login(c *gin.Context){  //登陆
	...
	user,err := model.FindUser(data.Account,data.Password)
	if err != nil {
		c.JSON(http.StatusBadRequest,gin.H{
			"msg": "查询出错",
			"err":err.Error(),
...
func UpdateUser(c *gin.Context){
	...
	//if queryResult,err :=model.FindNew(data.ID);err != nil{
	//	c.JSON(http.StatusBadRequest,gin.H{
	//		"msg":"根据id索引查询失败",
	//		"err":err.Error(),
	//	})
	...
	if err :=model.UpdateUser(data);err != nil{
		c.JSON(http.StatusBadRequest,gin.H{
			"msg": "更新出错",
			"err": err.Error(),
		})
...
func Verify(c *gin.Context){ //token验证登陆,同时返回user信息
	...
	user,err := model.FindUser(claims.Account,claims.Password)
	if err != nil {
		c.JSON(http.StatusBadRequest,gin.H{
			"msg": "查询出错",
			"err":err.Error(),
